    ...
    if (_freezeDir) {
        ...
        if (inlineTypes[_freezeDir]) {
            ...
            if (_freezeDir === ':base64:') {
                ...
                var encodedContent = encodeFunc(new Buffer(content).toString('utf8'))
                    .replace(/%20/g, ' ')
                    .replace(/#/g, '%23');
                if (_freezeDir === ':encodeURIComponentSvg:') {
                    encodedContent = encodedContent
                        ...
                        .replace(/(%[0-9A-Z]{2})/g, function(matched, AZ) {
                            return AZ.toLowerCase();
                        });
                }
                return inlineDecl + ',' + encodedContent;
            }
